...
class _BottomNavigationScreenState extends State<BottomNavigationScreen> {
  ...
  @override
  Widget build(BuildContext context) {// this line code need to recall when user click
  ...
  print("when relaod time this calling $currentIndex index"); 
    return  Container(
      ...
      decoration: BoxDecoration(image: DecorationImage(image: AssetImage(AppImages.app_background_image),fit: BoxFit.fitWidth)),
      child: Scaffold(
        ...
        body:screens[currentIndex] , //currenmt index =1
         bottomNavigationBar: Container(
            ...
            child: Row(
              ...
              children: [
                build_bottom_navigation("Home", 0,currentIndex,Icons.language,action:(){
                  setState(() {
                  });
             currentIndex=0;
                }),
                build_bottom_navigation("Favorites",1,currentIndex, Icons.favorite,action: () {
                  setState(() {
                  });
                  currentIndex=1;
                },),
                build_bottom_navigation("Profile", 2,currentIndex,Icons.person,action: (){
                  setState(() {
                  });
                  currentIndex=2;
                }),
...
Widget build_bottom_navigation(String title,int index,int currentIndex, IconData icon,{ required VoidCallback action}) {
  return InkWell(
    onTap: action ,
    child: Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [ //currentindx currnt value 1 index value ofcousre 1 then 
        Icon(icon, color:currentIndex==index? Colors.blue:Colors.white, size: 30),
        Text(
          title,
          style: TextStyle(
